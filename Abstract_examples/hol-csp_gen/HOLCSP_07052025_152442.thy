...
[simp del] :\<open>Trans_stm1\<cdot>n = 
	((((((((n_ = i0) \<^bold>& (((internal__stm1\<^bold>.NID_i0_stm1 \<rightarrow> Skip)\<^bold>;  (enter_s0_stm1 \<rightarrow> Trans_stm1\<cdot>s0_stm1))))
	  \<box>
	  ((n_ = s0) \<^bold>& (((a__in\<^bold>.NID_s0_stm1 \<rightarrow> Skip)\<^bold>;  ((SSTOP \<triangle> (exit_stm1 \<rightarrow> Skip))\<^bold>;  (SSTOP \<triangle> ((exited_stm1 \<rightarrow> Skip)\<^bold>;  (enter_s1_stm1 \<rightarrow> Trans_stm1\<cdot>s1_stm1))))))))
	  \<box>
	  ((n_ = s1) \<^bold>& (((b__in\<^bold>.NID_s1_stm1 \<rightarrow> Skip)\<^bold>;  ((SSTOP \<triangle> (exit_stm1 \<rightarrow> Skip))\<^bold>;  (SSTOP \<triangle> ((exited_stm1 \<rightarrow> Skip)\<^bold>;  (enter_s0_stm1 \<rightarrow> Trans_stm1\<cdot>s0_stm1))))))))
	  \<box>
	  ((n_ = s1) \<^bold>& (((internal__stm1\<^bold>.NID_s1_stm1 \<rightarrow> Skip)\<^bold>;  ((SSTOP \<triangle> (exit_stm1 \<rightarrow> Skip))\<^bold>;  (SSTOP \<triangle> ((exited_stm1 \<rightarrow> Skip)\<^bold>;  (enter_f0_stm1 \<rightarrow> Trans_stm1\<cdot>f0_stm1))))))))
	  \<box>
	  (share \<rightarrow> Trans_stm1\<cdot>n))
	  \<box>
