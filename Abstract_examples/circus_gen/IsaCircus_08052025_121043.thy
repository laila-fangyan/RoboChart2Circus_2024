...
" Trans_stm4 = 
	((SSTOP \<triangle> (get_v1\<^bold>?v1 \<rightarrow> (((((((n = NIDS_i0_stm4) \<^bold>& (((internal__stm4\<^bold>.NID_i0_stm4 \<rightarrow> (SSTOP \<triangle> (set_v1\<^bold>!1 \<rightarrow> Skip)));; (enter_s0_stm4 \<rightarrow> Trans_stm4(NIDS_s0_stm4)))))
	  \<box>
	  ((n = NIDS_s0_stm4) \<^bold>& ((((v1 \<ge> 1)) \<^bold>& (((internal__stm4\<^bold>.NID_s0_stm4 \<rightarrow> Skip);; ((SSTOP \<triangle> (exit_stm4 \<rightarrow> Skip));; (SSTOP \<triangle> ((exited_stm4 \<rightarrow> Skip);; (enter_s1_stm4 \<rightarrow> Trans_stm4(NIDS_s1_stm4)))))))))))
	  \<box>
	  ((n = NIDS_s1_stm4) \<^bold>& ((((v1 < 1)) \<^bold>& (((internal__stm4\<^bold>.NID_s1_stm4 \<rightarrow> Skip);; ((SSTOP \<triangle> (exit_stm4 \<rightarrow> Skip));; (SSTOP \<triangle> ((exited_stm4 \<rightarrow> Skip);; (enter_s0_stm4 \<rightarrow> Trans_stm4(NIDS_s0_stm4)))))))))))
	  \<box>
	  ((n = NIDS_s1_stm4) \<^bold>& (((a__in\<^bold>.NID_s1_stm4 \<rightarrow> Skip);; ((SSTOP \<triangle> (exit_stm4 \<rightarrow> Skip));; (SSTOP \<triangle> ((exited_stm4 \<rightarrow> Skip);; (enter_s0_stm4 \<rightarrow> Trans_stm4(NIDS_s0_stm4)))))))))
	  \<box>
	  (share \<rightarrow> Trans_stm4(n)))
	  \<box>
	  (((interrupt_stm4 \<rightarrow> (SSTOP \<triangle> (exit_stm4 \<rightarrow> Skip)));; (SSTOP \<triangle> (exited_stm4 \<rightarrow> (terminate \<rightarrow> Skip))))
	  \<box>
	  (terminate \<rightarrow> Skip)))))) "
...
" ncCoreBehaviour_stm4 = 
	(((composeNodes_stm4 [ interrupt_i0_stm4 \<mapsto> internal__stm4\<cdot>NID_i0_stm4,  
	 ...
	 interrupt_s0_stm4 \<mapsto> interrupt_stm4,  
	 interrupt_s1_stm4 \<mapsto> interrupt_stm4 ]) [ share \<mapsto> share ])
