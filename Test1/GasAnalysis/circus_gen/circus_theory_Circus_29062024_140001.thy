...
"Termination = (terminate \<rightarrow> Skip)" | 
"Active = ((SSTOP \<triangle> (get_gs?(gs) \<rightarrow> (intensity(gs) & ((SSTOP \<triangle> (set_ins!intensity(gs) \<rightarrow> Skip))))));; (Behaviour;; Exiting))" | 
...
"Termination = (terminate \<rightarrow> Skip)" | 
"Active = ((SSTOP \<triangle> (get_gs?(gs) \<rightarrow> (analysis(gs) & ((SSTOP \<triangle> (set_sts!analysis(gs) \<rightarrow> Skip))))));; (Behaviour;; Exiting))" | 
