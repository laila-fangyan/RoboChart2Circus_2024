...
definition "enteredSS_GasAnalysis_Chemical = \<lbrace> 
	  ...
	  entered_j1_GasAnalysis_Chemical \<rbrace>"
...
"Termination = (terminate \<rightarrow> Skip)" | 
"Active = ((SSTOP \<triangle> (get_gs?(gs) \<rightarrow> (((size(gs) > 0) \<and> (3 > 0)) & ((SSTOP \<triangle> (set_ins!intensity(gs) \<rightarrow> Skip))))));; (Behaviour;; Exiting))" | 
...
"During = (((SSTOP \<triangle> (get_gs?(gs) \<rightarrow> ((size(gs) > 0) & ((SSTOP \<triangle> (set_sts!analysis(gs) \<rightarrow> Skip))))));; SSTOP) \<triangle> (interrupt_Analysis_GasAnalysis_Chemical \<rightarrow> Skip))" | 
"Exiting = ((SSTOP \<triangle> (exit_GasAnalysis_Chemical \<rightarrow> Skip));; (Skip;; (exited_GasAnalysis_Chemical \<rightarrow> Inactive)))"
